...
public class GameScreen extends GLScreen {
	...
	public GameScreen(Game game) {
		...
		captureBounds = new Rectangle(252,90,64,64);
		gameoverBounds = new Rectangle(240, 160, 128, 64);
	...
	private void updatePaused() {
		...
		for (int i = 0; i < len; i++) {
			...
			if (OverlapTester.pointInRectangle(resumeBounds, touchPoint)) {
				...
				((DroidInvaders) game).hideAppucinoBar();
				state = GAME_RUNNING;
			}
	...
	private void updateRunning(float deltaTime) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (OverlapTester.pointInRectangle(pauseBounds, touchPoint)) {
				Assets.playSound(Assets.clickSound);
				((DroidInvaders) game).showAppucinoBar();
				state = GAME_PAUSED;
			}
		...
		if (world.isGameOver()) {
			((DroidInvaders) game).updateScoreAppucino((long) lastScore, "Default");
			((DroidInvaders) game).showAppucinoBar();
			state = GAME_OVER;
		}
	}
	...
	private void updateGameOver() {
		...
		for (int i = 0; i < len; i++) {
			...
			if (event.type == TouchEvent.TOUCH_UP) {
				Assets.playSound(Assets.clickSound);
				guiCam.touchToWorld(touchPoint.set(event.x, event.y));
				if (OverlapTester.pointInRectangle(captureBounds, touchPoint)) {
					((DroidInvaders) game).doScoreCheckin();
				}
	...
	private void presentGameOver() {
		...
		batcher.drawSprite(252,90,64,64,Assets.captureRegion);
		Assets.font.drawText(batcher, scoreString, 10, 320 - 20);
		batcher.endBatch();
